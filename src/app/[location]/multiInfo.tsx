...
export default function MultiInfo(infoProps: MultiInfoProps) {
  ...
  const formattedInfo = (
    element: "clouds" | "pressure" | "timezone" | "sunrise" | "sunset",
    value: number
  ) => {
    switch (element) {
      case "clouds":
      ...
      case "sunset":
      case "sunrise":
        return `${parseLocalTime(value + info.timezone.value)}`;
      case "timezone":
    ...
    <div className="flex flex-col w-full">
          ...
          Object.entries(publicProperties).map(([title, obj], index) => {
            ...
            return (
              <motion.div
                ...
                className={`${
                  !enable && "hidden"
                } font-poppins w-[150px] backdrop-brightness-125 backdrop-blur p-4 rounded-xl border-white border-2
               border-opacity-20 `}
              >
                <p className="first-letter:uppercase text-gray-300">{title}</p>
      ...
      <DivToggle
        icon={
          <motion.svg
            ...
            className="p-1 cursor-pointer hover:rotate-180 duration-200"
            fill="#ffffff"
            version="1.1"
            id="Capa_1"
      ...
      >
        <div
          ...
          className="bg-gray-200 p-7 flex flex-col gap-5 text-black text-center "
        >
